...
const generateAcessAndRefreshTokens = (pyload) => {
  const accessToken = jwt.sign(pyload, process.env.JWT_SECRET, {
    expiresIn: "60m",
  });
  const refreshToken = jwt.sign(pyload, process.env.JWT_SECRET_REFRESH, {
    expiresIn: "7d",
  });
...
export const updateAccount = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ status: "Internal server error in updating", error: error });
  }
};
